...
use crate::scene::types::{DirectionalLight, PointLight};
...
impl LightRenderer {
    ...
    pub fn prepare(&mut self, world: &hecs::World) {
        let mut binding = world.query::<&DirectionalLight>();
        let iter = binding.into_iter();

        self.queue.write_buffer(
            &self.directional_light_count_buffer,
            0,
            bytemuck::bytes_of(&(iter.len() as u32)),
        );
        for (i, directional_light) in iter.enumerate() {
            if i >= Self::MAX_DIRECTIONAL_LIGHTS as usize {
                warn!("Max number of directional lights reached! no more will be rendered.");
                return;
            }

            let shader_light: ShaderDirectionalLight = directional_light.into();
            self.queue.write_buffer(
                &self.directional_lights_buffer,
                size_of::<ShaderDirectionalLight>() as u64 * (i as u64),
                bytemuck::bytes_of(&shader_light),
            );
            self.shadow_renderer
                .update_directional_light_projection(i as u32, &shader_light.projection);
        }
    }
