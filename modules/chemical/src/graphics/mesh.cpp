...
namespace Chemical {
	...
	namespace Graphics {
		...
		StaticMesh2D CreateStaticMesh2D(const std::vector<float> vertices, const std::vector<unsigned int> indices) {
			...
			glNamedBufferStorage(VBO, sizeof(float) * vertices.size(), &vertices[0], GL_DYNAMIC_STORAGE_BIT);

			unsigned int IBO = 0;
			glCreateBuffers(1, &IBO);
			glNamedBufferStorage(IBO, sizeof(unsigned int) * indices.size(), &indices[0], GL_DYNAMIC_STORAGE_BIT);
