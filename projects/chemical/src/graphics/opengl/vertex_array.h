...
namespace Chemical {
	...
	namespace OpenGL {
		...
		struct InstancedElementDrawInfo {
			...
			DataType data_type = DataType::UNSIGNED_INT; // must be uint, ushort, ubyte
			int32_t count = 0;
			int64_t offset = 0;
			int32_t instance_count = 0;

			InstancedElementDrawInfo(int32_t count, int32_t instance_count, int64_t offset = 0, DataType data_type = DataType::UNSIGNED_INT, DrawMode mode = DrawMode::TRIANGLES) :
				count(count), instance_count(instance_count), offset(offset), data_type(data_type), mode(mode) {}
		...
		struct ElementDrawInfo {
			...
			ElementDrawInfo(uint32_t count, int64_t offset = 0, DataType data_type = DataType::UNSIGNED_INT, DrawMode mode = DrawMode::TRIANGLES) :
				count(count), offset(offset), data_type(data_type), mode(mode) {}
		...
		struct InstancedArrayDrawInfo {
			...
			InstancedArrayDrawInfo(uint32_t count, uint32_t instance_count, int32_t first = 0, DrawMode mode = DrawMode::TRIANGLES) :
				count(count), instance_count(instance_count), first(first), mode(mode) {}
		...
		*/
		...
		public:
			...
			void SetVertexBuffer(const Buffer& buffer, const VertexLayout& info, int64_t offset, uint32_t binding_index);
